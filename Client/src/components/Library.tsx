...
const Library = () => {
  ...
  const [filteredRecommendedCoursesBooks, setFilteredRecommendedCoursesBooks] =
    useState<any>();
  ...
  const filterRecommendedBooks = (subjects: string[]) => {
    ...
    setFilteredBooks(
      uniqueBooks.length > 0
        ? uniqueBooks
        : [{ name: "No Book found as same of your subjects", author: "" }]
    );
  };

  const filterRecommendedCoursesBooks = (courses: any) => {
    //@ts-ignore
    const courseBooks = courses.reduce((acc, course) => {
      ...
      acc[course] =
        filtered.length > 0
          ? filtered
          : [{ name: "No Books found for your course ", author: "" }];
      return acc;
    }, {});
    ...
    <div className="divClassName">
      ...
      <div className="percent">
        <p className="percent-p d-flex justify-content-between align-items-center">
          Select the Course
          <button
            className="btn bg-primary mx-5 text-white"
        ...
        <span className="ml-3">
          <button
            className="btn btn-outline-success w-30 m-1 fw-bold"
          ...
          {uniqueCourses.map((course) => (
            <button
              key={course}
              className="btn btn-outline-primary w-40 mx-1 fw-bold"
            ...
            <div key={course}>
              ...
              <div className="table-container mt-0">
                <table className="table-custom">
                  ...
                  <tbody>
                    {Array.isArray(filteredRecommendedCoursesBooks[course]) &&
                      filteredRecommendedCoursesBooks[course].map(
                        (book, index) => (
                          <tr key={index}>
                            <td>{book.name}</td>
                            <td>{book.author}</td>
